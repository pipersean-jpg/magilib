...
// ── CSS ────────────────────────────────────────────────────────
var _CSS = [
  /* ── Floating bar ── always visible when catalog is shown ── */
  '.bk-bar{',
  ...
  '}',
  /* Idle state: just the Select pill */
  '.bk-bar-idle{',
    'background:rgba(42,31,107,0.10);',
    'border:1.5px solid rgba(42,31,107,0.20);',
  ...
  '}',
  '.bk-bar-idle:hover{background:rgba(42,31,107,0.16);}',
  /* Active state: dark pill with actions */
  '.bk-bar-actions{',
    'display:none;',
  ...
  '}',
  '.bk-bar-actions.on{display:flex;}',
  '.bk-cnt{font-size:12px;font-weight:600;opacity:.7;white-space:nowrap;padding:0 4px;}',
  ...
  /* Delete All button inside settings */
  '.bk-del-all-btn{',
...
// ── DOM ────────────────────────────────────────────────────────
var _bar,_idleBtn,_actionsRow,_banner;

function _buildDOM(){
  if(_bar)return;

  // Outer wrapper (always fixed bottom-right)
  _bar=document.createElement('div');
  _bar.id='_bkBar';_bar.className='bk-bar';

  // Idle pill — visible whenever catalog is showing
  _idleBtn=document.createElement('button');
  _idleBtn.className='bk-bar-idle';
  _idleBtn.id='_bkIdle';
  _idleBtn.textContent='\u2611 Select';
  _idleBtn.onclick=function(){_on?_bkExit():_bkEnter();};
  _bar.appendChild(_idleBtn);

  // Actions row — shown once in select mode
  _actionsRow=document.createElement('div');
  _actionsRow.className='bk-bar-actions';
  _actionsRow.id='_bkActions';
...
// ── Show/hide bar based on which view is active ────────────────
function _barVisible(show){
  if(_bar)_bar.style.display=show?'flex':'none';
}
...
function _sync(){
  if(!_bar)return;
  var n=_sel.size;
  // Actions row only visible when in select mode
  if(_actionsRow)_actionsRow.classList.toggle('on',_on);
  // Idle button: relabel when in select mode
  if(_idleBtn)_idleBtn.textContent=_on?'\u2715 Exit Select':'\u2611 Select';
  // Count
...
function _injectSettingsBtn(){
  ...
  btn.title='Permanently delete all books (3 confirmations required)';
...
// ── Show bar only on catalog view ─────────────────────────────
// Watches for view switches by observing the active class on views
function _watchViews(){
  var catalog=document.getElementById('view-catalog');
  var settings=document.getElementById('view-settings');
  if(!catalog)return;

  var obs=new MutationObserver(function(){
    var onCatalog=catalog.classList.contains('active');
    _barVisible(onCatalog);
    // Try to inject settings button whenever settings view becomes active
    if(settings&&settings.classList.contains('active')){
      _settInject=0;
      setTimeout(_injectSettingsBtn,200);
    }
  });
  obs.observe(catalog,{attributes:true,attributeFilter:['class']});
  // Initial state
  _barVisible(catalog.classList.contains('active'));
}
...
setTimeout(function(){
  ...
  window.renderCatalog=function(){
    ...
    setTimeout(function(){
      _css();_buildDOM();_watchViews();
      if(_on)_stamp();
      _sync();
      // Also try settings inject in case settings tab is already open
      if(!document.getElementById('_bkDelAllBtn')){
        _settInject=0;setTimeout(_injectSettingsBtn,200);
      }
    },0);
  };
},120);

// Also attempt settings inject on a small delay after page load,
// in case the user is already on settings when this runs
setTimeout(function(){
  _css();_buildDOM();_watchViews();
  _settInject=0;_injectSettingsBtn();
},800);
