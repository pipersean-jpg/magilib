...
var _CSS = [
  '.bk-bar{',
    ...
    'font-family:inherit;',
  '}',
  '.bk-bar.on{display:flex;}',
  '.bk-cnt{font-size:12px;font-weight:600;flex:1;white-space:nowrap;opacity:.75;}',
  ...
  '.bk-b.exit{padding:5px 8px;opacity:.55;}',
  '.bk-b.exit:hover{opacity:1;}',
  /* Banner */
  '.bk-banner{display:none;align-items:center;gap:8px;',
    'padding:4px 0;font-size:12px;font-weight:600;color:var(--ink,#1A1814);}',
  ...
  /* Transparent click-capture overlay on each card */
...
function _buildDOM(){
  ...
  _bar.className='bk-bar';_bar.id='_bkBar';
  _bar.innerHTML=
...
// ── Find the toolbar ───────────────────────────────────────────
// Tries multiple selectors. As final fallback, inserts after the
// first .filter-chip it can find (which is always present).
function _tb(){
  var sel=[
    '#catalogActions',
    '.catalog-actions',
    ...
    '[class*="cat-toolbar"]',
    '.filter-chips',
    '[class*="filter-chip"]'  // last resort: a chip itself
  ];
  for(var i=0;i<sel.length;i++){
    var el=document.querySelector(sel[i]);
    if(el){
      // If we landed on a single chip, return its parent row
      if(el.classList.contains('filter-chip')||el.classList.contains('filter-chips')){
        return el.closest('div')||el.parentElement;
      }
      return el;
    }
  }
  // Absolute fallback: parent of the first filter-chip button
  var chip=document.querySelector('button.filter-chip, .filter-chip');
  return chip ? chip.parentElement : null;
}

// Find where to INSERT the Select button.
// We want it in the top toolbar row (same row as Refresh Library),
// not in the filter chips row below.
function _topBar(){
  // The Refresh Library button is a reliable anchor
  var refresh=document.querySelector('button[onclick*="loadCatalog"], button[onclick*="refreshLibrary"], #refreshLibraryBtn');
  if(refresh)return refresh.parentElement;
  // view-catalog header area
  var header=document.querySelector('#view-catalog .catalog-header, #view-catalog > div:first-child');
  if(header)return header;
  return _tb(); // fall through to filter row if nothing else found
}
...
// ── Stamp cards ────────────────────────────────────────────────
// Uses a transparent overlay to capture taps without fighting
...
function _stamp(){
  document.querySelectorAll('.book-card').forEach(function(card,i){
    ...
    if(_on){
      ...
      // Visual checkbox (plain div, not input — avoids focus/event conflicts)
...
function _bkEnter(){
  ...
  _on=true;_sel.clear();
  _stamp();
  // Place banner just above the books grid
  var grid=document.getElementById('booksGrid');
  if(grid&&_banner){grid.before(_banner);_banner.classList.add('on');}
  if(_togBtn){_togBtn.textContent='\u2715 Exit Select';_togBtn.style.background='rgba(42,31,107,.08)';}
  _sync();
}
function _bkExit(){
  ...
  if(_togBtn){_togBtn.textContent='\u2611 Select';_togBtn.style.background='';}
  _stamp();_sync();
}
...
// Retries up to 10 times with 300ms gaps in case DOM isn't ready yet
var _injectAttempts=0;
function _injectBtns(){
  if(document.getElementById('_bkTogBtn'))return; // already done
  _injectAttempts++;

  var anchor=_topBar();
  if(!anchor){
    if(_injectAttempts<10)setTimeout(_injectBtns,300);
    return;
  }
  ...
  _togBtn.style.cssText=
    ...
    'cursor:pointer;color:var(--ink,#1A1814);font-weight:600;'+
  ...
  db.id='_bkDelAll';
  db.textContent='Delete All';
  db.title='Delete entire library (3 confirmations required)';
  db.style.cssText=
    'font-size:13px;padding:6px 14px;border-radius:8px;'+
    'border:1.5px solid rgba(200,60,60,.3);background:transparent;'+
    'cursor:pointer;color:rgba(170,40,40,.85);font-weight:600;'+
    'transition:all .15s;white-space:nowrap;font-family:inherit;'+
    'margin-left:6px;';
  db.onclick=_bkDelAll;

  anchor.appendChild(_togBtn);
  anchor.appendChild(db);
}
...
setTimeout(function(){
  ...
  window.renderCatalog=function(){
    ...
    setTimeout(function(){
      _css();_buildDOM();
      // Reset inject attempts so it retries finding the toolbar after each render
      if(!document.getElementById('_bkTogBtn')){
        _injectAttempts=0;
        _injectBtns();
      }
      if(_on)_stamp();
