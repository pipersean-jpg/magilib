...
// ── CSS ───────────────────────────────────────────────────────
// Bar starts display:none — only shown when _on===true AND n>0
var _CSS = [
  '.bk-bar{',
    'display:none;', /* hidden until needed */
    'position:fixed;bottom:68px;left:50%;',
    'transform:translateX(-50%);',
    'background:rgba(30,24,70,0.92);',
  ...
  '.bk-cnt{font-size:12px;font-weight:600;flex:1;white-space:nowrap;opacity:.8;}',
  '.bk-b{',
    ...
    'font-family:inherit;',
  '}',
  '.bk-b:hover{background:rgba(255,255,255,.22);}',
  '.bk-b.red{background:rgba(220,60,60,.25);}',
  '.bk-b.red:hover{background:rgba(220,60,60,.45);}',
  '.bk-b.gold{background:rgba(201,168,76,.22);}',
  '.bk-b.gold:hover{background:rgba(201,168,76,.42);}',
  '.bk-b.exit{padding:5px 7px;opacity:.6;}',
  '.bk-b.exit:hover{opacity:1;background:rgba(255,255,255,.12);}',
  /* Banner below toolbar */
  '.bk-banner{',
    'display:none;',
    'background:rgba(42,31,107,.08);',
    'border:1px solid rgba(42,31,107,.15);',
  ...
  '.bk-all{',
    'font-size:11px;',
    'background:rgba(42,31,107,.1);',
    'border:none;',
    'color:var(--accent,#2A1F6B);',
    'padding:3px 8px;border-radius:5px;cursor:pointer;font-family:inherit;',
  '}',
  /* Cards in select mode */
  '.book-card.bk-mode{cursor:pointer!important;}',
  ...
  '.bk-cb{',
    ...
    'font-size:13px;font-weight:700;color:#2A1F6B;',
    'transition:background .12s,border-color .12s;',
    'line-height:1;',
  '}',
  '.bk-cb.checked{background:#2A1F6B;border-color:#2A1F6B;color:#fff;}',
  /* Overlay to intercept all card clicks in select mode */
  '.bk-overlay{',
...
function _buildDOM(){
  ...
  _bar.innerHTML=
    ...
    '<button class="bk-b exit" onclick="_bkExit()" title="Exit select mode">\u2715</button>';
...
function _sync(){
  if(!_bar)return;
  var n=_sel.size;
  // Bar: only show when in select mode AND at least 1 selected
  if(_on && n>0){
    _bar.classList.add('on');
  } else {
    _bar.classList.remove('on');
  }
  var countEl=document.getElementById('_bkCount');
...
// ── Stamp cards ────────────────────────────────────────────────
// Uses a transparent overlay div to intercept clicks in select mode.
// This avoids fighting with inline onclick= attributes on the cards.
function _stamp(){
  document.querySelectorAll('.book-card').forEach(function(card,i){
    ...
    // Remove any existing overlay and checkbox
    var old=card.querySelector('.bk-overlay');
    if(old)old.remove();
    var oldcb=card.querySelector('.bk-cbw');
    if(oldcb)oldcb.remove();

    if(_on){
      card.classList.add('bk-mode');

      // Transparent click overlay — sits above the card content,
      // captures the tap before the card's own onclick fires
      var ov=document.createElement('div');
      ov.className='bk-overlay';
      // Store bid in closure via data attr
      ov.dataset.bid=bid;
      ...
      // Visual checkbox (display-only div, not an input)
      var w=document.createElement('div');
      w.className='bk-cbw';
      var cb=document.createElement('div');
      cb.className='bk-cb'+(_sel.has(bid)?' checked':'');
      cb.textContent=_sel.has(bid)?'\u2713':'';
      w.appendChild(cb);
...
// Update just the visual state of cards without re-stamping
...
function _injectBtns(){
  ...
  db.style.cssText=
    ...
    'cursor:pointer;color:rgba(180,50,50,.8);font-weight:600;'+
    'transition:all .15s;white-space:nowrap;font-family:inherit;';
